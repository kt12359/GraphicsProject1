...
{
	int i = 0, j = 0, realWidth = width * 4, size = width * height * 4, count = 0, toMove = 0, rowOffset = 0;
	To_Grayscale();
	float * output = new float [size];
	for(i = 0; i < size; ++i)
		output[i] = 0;
	for(i = 0; i < size; i+= 4)	{				//convert to float in range 0-1, ignore alpha
		output[i] = output[i+GREEN] = output[i+BLUE] = (float)(data[i]/(float)256);
	}
	for(i = 0; i < height; ++i)
	{
		for(j = 0; j < width ; ++j)
		...
		for(i = 0; i < size; i+= 4)				//convert to float in range 0-1, ignore alpha
		data[i] = data[i+GREEN] = data[i+BLUE] = output[i];
	delete [] output;
    return true;
}// Dither_FS
...
{ 
	int left, down, downL, downR, realWidth = width * 4, rowOffset = i * width * 4; 
 	left = i * realWidth + (realWidth-4-(j*4) + 4); 
	if((j*4) >= width-4)  
		left = -7; 
	down = (i+1)*realWidth+(realWidth-4-(j*4)); 
	downL = (i+1)*realWidth+(realWidth-4-(j*4)+4); 
	downR = (i+1)*realWidth+(realWidth-4-(j*4)-4); 
	if(i+1 >= height){ 
		down = -7; 
		downL = -7; 
		downR = -7; 
	...
	if((j*4) >= realWidth)
		downL = -7; 
	int currIndex = rowOffset + (realWidth-4-(j*4)); 
	calcThresh(currIndex,output,left,down,downR,downL); 
}
